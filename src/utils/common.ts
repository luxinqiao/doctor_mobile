...
*/
const priceTransitionYuanFen = (price: string)=> {
    if(isEmpty(price)) {
        return {
            integer: '',
            decimal: ''
    ...
    let arr = (Number(price)/100).toFixed(2).split('.')
	return {
        integer: arr[0],
        decimal: arr[1]
    }
 ...
 */
const convertSeconds = (timestamp:number) => {
    if(timestamp<=0) {
        return {
            h:'00',s:'00',m:'00'
        }
    ...
    let s = Math.floor((timestamp % 60)) < 10 ? '0' + Math.floor((timestamp % 60)) : Math.floor((timestamp % 60))
    return {
        h,m,s
    }
}
