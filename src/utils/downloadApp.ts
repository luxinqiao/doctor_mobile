import { getBrowserType, getIosVersion} from '@/utils/common'
...
const downloadApp = ()=> {
    if (appType == 'ios') {
        window.location.href = `${doc_ios}&timestamp=${new Date().getTime()}`
    } else {
        if ( browserType == 'wechat') {
            window.location.href = `${doc_android_wc}&timestamp=${new Date().getTime()}`
        } else {
            window.location.href = `${doc_android}&timestamp=${new Date().getTime()}`
        }
    }
}
