    ...
    <div id='invite' :class='stateSkin' :style='"padding-top:"+top+"px"'>
        ...
        <div class='box'>
            ...
            <div class='top_desc' v-show='desc'>
                <div>每邀请1位用户注册并登录澜渟App账号，可获得3天VIP会员奖励。</div>
            </div>
...
<script lang='ts'>
    ...
    export default class inviteUserIndex extends Vue {
        ...
        private shareParam: any = {
            url: `${(this as any).$request.user}/pullNew/sign?referral_code=${this.$route.query.referral_code}&type=LT15`,
            img: '',
            uploading: false
        }
        private poster: any = {
        ...
        mounted() {
            if (this.$route.query.from != 'activity') {
        ...
        initPoster(name: string) {
            ...
            this.poster.datas.push({
                type: 'img',
                content: require("@/assets/img/pullNew/invite/user/index/poster_bg.png"),
                pos: {x: 0, y: 0},
                size: {w: 29, h: 43.2}
            })
            //昵称
        ...
        share(type:number) {
            ;(this as any).$axios.post(`${(this as any).$request.live}/v1/click`, {
                module: 'pull',
                event: type==0?'lt_qr_code_wx_friend': (type==1?'lt_qr_code_wx_circle': (type==2?'lt_qr_code_save_img': 'lt_qr_code_copy_url'))
            }).then((res: any) => {})
        }
    }
</script>
